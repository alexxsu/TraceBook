  ...
  const handleShareAsImage = async () => {
    ...
    try {
      ...
      canvas.toBlob(async (blob: Blob | null) => {
        ...
        const file = new File([blob], `${place.name.replace(/\s+/g, '_')}_Experience.png`, { type: 'image/png' });
        ...
        if (navigator.share) {
          try {
            await navigator.share({
              files: [file],
              title: place.name,
              text: `Check out my experience at ${place.name} on TraceBook!`
            });
          } catch (shareError) {
          ...
          >
            ...
            >
              ...
              {place.visits.length > 0 && (
                <img
                  src={place.visits[0].photoDataUrl}
              ...
              <div className="absolute bottom-4 left-4 right-4 z-20 pointer-events-none">
                <h1 className="text-xl sm:text-2xl font-bold text-white leading-tight drop-shadow-lg">{place.name}</h1>
                <div className="flex items-center gap-1.5 text-gray-200 text-xs mt-1.5">
                  <MapPin size={12} className="flex-shrink-0" />
                  <span className="truncate drop-shadow">{place.address}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex border-b border-gray-800/80 bg-gray-900/80 backdrop-blur-sm">
              <div className="flex-1 p-3 text-center border-r border-gray-800/80">
                <span className="block text-lg font-bold text-white">{place.visits.length}</span>
               ...
               <div className="bg-gray-800 rounded-xl p-4 border border-gray-700">
                 ...
                 <div className="space-y-4">
                   ...
                   <div>
                     ...
                     <p className="text-white font-medium">{place.address}</p>
                   </div>
