...
const FireworkParticle: React.FC<{
  ...
  endX: number;
  endY: number;
}> = ({ delay, startX, startY, color, size, endX, endY }) => {
  return (
    <div
    ...
    for (const burst of burstPoints) {
      ...
      for (let i = 0; i < particleCount; i++) {
        ...
        bursts.push({
          ...
          startX: burst.x,
          startY: burst.y,
          delay: burst.delay + Math.random() * 0.15,
...
export const Tutorial: React.FC<TutorialProps> = ({
  ...
  useEffect(() => {
    ...
    // Close UI elements before certain steps
    if (currentStep === 'filter_button') {
      onCloseMapManagementRef.current?.();
    }

    if (currentStep === 'side_menu') {
      onCloseMapManagementRef.current?.();
    }

    if (currentStep === 'map_pill') {
      onCloseMapManagementRef.current?.();
    }

    if (currentStep === 'map_controls') {
      onCloseMapManagementRef.current?.();
    }

    if (currentStep === 'add_button') {
      onCloseMapManagementRef.current?.();
    }
  ...
  useEffect(() => {
    if (isActive && currentStep === 'map_management') {
      setMapManagementReady(false);
  ...
  useEffect(() => {
    ...
    if (!config?.autoAdvanceDelay) return;
  ...
  }, [isActive, currentStep, currentStepIndex, steps, goToNextStep]);
  ...
  }, [isActive, currentStep, currentConfig?.targetSelector]);
