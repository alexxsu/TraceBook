...
const AddVisitModal: React.FC<AddVisitModalProps> = ({ 
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isGuest) {
        // For guest users: Convert blobs to base64 data URLs (stored locally only)
        const base64Promises = previewBlobs.map(blob => {
        ...
        photoUrls = await Promise.all(base64Promises);
      } else {
